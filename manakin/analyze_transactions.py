...
class View_details():
    ...
    def __init__(self, raw_datas, columns_to_scatter=COLUMNS_TO_SCATTER, graph_path=GRAPH_PATH):  # MODEL
        ...
        class Montant_plot():  # VUE
            ...
            def update(self, montants_dict):
                ...
                logging.info('ind: %s means: %s yerr: %s', str(ind), str(means), str(stds))
                self.subplot.clear()
    ...
    def update(self, idxs):  # CONTROLEUR
        ...
        logging.info("View_details: updating with %s accounts", str(len(idxs)))

        labels = [ self.raw_datas[idx][label_column] for idx in idxs ]
        labels_count = Counter(labels)

        commons = np.array(labels_count.most_common())[:, 0]
        logging.info("view_details: only display class %s", str(commons))
